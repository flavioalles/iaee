...
if [ -d "$HOME/.iaee/.master/" ]
then
   rm -rf $HOME/.iaee/.master
fi
mkdir $HOME/.iaee/.master
if [ -d "$HOME/.iaee/.client/" ]
then
   rm -rf $HOME/.iaee/.client
fi
mkdir $HOME/.iaee/.client
echo "Creating output files (master & client) in $HOME/.iaee..."
sleep 1
if [ -f $HOME/.iaee/.master/master.tty ]
then
   rm $HOME/.iaee/.master/master.tty
fi
touch $HOME/.iaee/.master/master.tty
if [ -f $HOME/.iaee/.client/client-$UNAME.tty ]
then
   rm $HOME/.iaee/.client/client-$UNAME.tty
fi
touch $HOME/.iaee/.client/client-$UNAME.tty
